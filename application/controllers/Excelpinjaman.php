...
class Excelpinjaman extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $sheet->setCellValue('A1', "BUKU PIUTANG");
        ...
        $this->db->where(['id_user' => $id_user]);
        $sql = $this->db->get('tb_pinjaman')->result();
        $a = 0;
        $b = 8;
        if ($sql) {
            foreach ($sql as $row) {
                ...
                if ($row->kode == "2") {
                    $kredit = $row->jumlah_bayar;
                    $debet = '';
                } else {
                    $debet = $row->jumlah_pinjam;
                    $kredit = '';
                }
                ...
                if (empty($klr->jumlah_pinjam)) {
                    ...
                    $debet2 = $klr->jumlah_pinjam;
                }
                $saldo = $debet2 - $kredit2;
        ...
        $sheet->setTitle("DATA BUKU PIUTANG");
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment; filename="Rekap Pinjaman.xlsx"');
